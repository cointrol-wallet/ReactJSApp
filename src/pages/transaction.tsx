...
export function Transactions() {
  ...
  const [txQrApplyCount, setTxQrApplyCount] = React.useState(0);
  ...
  const txQrHandled = React.useRef<object | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (!txQr) return;
    if (txQr === txQrHandled.current) return;
    if (cLoading || crLoading || fLoading) return;

    txQrHandled.current = txQr;
    ...
    if (txQr.args) {
      ...
      for (const [k, v] of Object.entries(txQr.args)) {
        ...
        if (EVM_ADDR_RE.test(v)) {
          addrFields[normalizedKey] = { mode: "manual", manual: v, selectedIndex: null };
        } else {
          plainArgs[normalizedKey] = v;
        }
      }
    ...
    if (txQr.type === "contract") {
      ...
      if (txQr.contractName === "QuantumAccount" && txQr.contractAddress) {
        ...
        const matchingFolio = folios.find(
          f => f.chainId === txQr.chainId &&
               f.address.toLowerCase() === txQr.contractAddress!.toLowerCase()
        );
        if (matchingFolio) setSelectFolio(matchingFolio);
        setTransferOrTransaction(false);
        setCardTitle("Use a Smart Contract");
        setCardDescription("Select any contract and then choose a function");
        setIsModalOpen(true);
  ...
  }, [location.state, cLoading, crLoading, fLoading, contracts, coins, folios, mainnetRpcUrl]);
